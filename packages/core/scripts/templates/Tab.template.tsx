...
export const Tab = React.forwardRef<HTMLButtonElement, TabProps>((props, ref) => {
  ...
  const effectiveState: TabState = disabled ? 'disabled' : controlledState ?? (active ? 'active' : internalState);
  ...
  }, [disabled, value, onSelect, onClick]);
  ...
  const focusRingClass = effectiveState === 'focus' || (!controlledState && !disabled)
    ? 'focus-visible:outline-none focus-visible:shadow-[var(--effect-focus-brand)]'
    : '';
