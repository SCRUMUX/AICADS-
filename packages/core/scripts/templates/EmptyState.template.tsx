...
const IconFrame: React.FC<IconFrameProps> = ({ node, frameSize, iconSize, colorClass }) => (
  <span
    ...
    aria-hidden="true"
  >
    <IconSlot icon={node} size={`${iconSize}px`} />
  </span>
);
...
export const EmptyState = React.forwardRef<HTMLDivElement, EmptyStateProps>((props, ref) => {
  ...
  const resolvedCta = ctaButton ?? (ctaLabel && onCtaClick ? <Button appearance="brand" size="sm" onClick={onCtaClick}>{ctaLabel}</Button> : null);
  const resolvedSecondary = secondaryButton ?? (secondaryLabel && onSecondaryClick ? <Button appearance="ghost" size="sm" onClick={onSecondaryClick}>{secondaryLabel}</Button> : null);

  const hasActions = (showCta && resolvedCta) || (showSecondary && resolvedSecondary);

  const actionsRow = hasActions ? (
    <div className={cn('flex flex-row items-center flex-wrap', actionsGap)}>
      {showCta     && resolvedCta      && <span className="shrink-0">{resolvedCta}</span>}
      {showSecondary && resolvedSecondary && <span className="shrink-0">{resolvedSecondary}</span>}
    </div>
  ) : null;
