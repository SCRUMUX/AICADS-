...
export const DropdownItem = React.forwardRef<HTMLDivElement, DropdownItemProps & { onClick?: () => void }>((props, ref) => {
  ...
  const recalcPos = useCallback(() => {
    ...
    const gap = 4;
    const submenuW = 180;
    const fitsRight = rect.right + gap + submenuW <= window.innerWidth;
    setSubmenuPos({
      top: rect.top,
      left: fitsRight ? rect.right + gap : rect.left - gap - submenuW,
    });
  }, []);
  ...
  return (
    ...
    >
      ...
      {hasSubmenu && submenuOpen && ReactDOM.createPortal(
        <div
          className="fixed z-[9999] min-w-[160px] bg-[var(--color-surface-1)] border border-[var(--color-border-base)] rounded-[var(--radius-default)] shadow-elevation-2 animate-dropdown-in"
          style={{ top: submenuPos.top, left: submenuPos.left, padding: 'var(--popover-pad, 6px)' }}
